...
def plot_spots(
    ...
    # Clip locs to ensure they are within image boundaries
    locs[:, 0] = np.clip(locs[:, 0], 0, img_h - 1)
    locs[:, 1] = np.clip(locs[:, 1], 0, img_w - 1)
    ...
    for ij, ct in zip(locs, cnts):
        ...
        valid_indices = (indices[:, 0] >= 0) & (indices[:, 0] < img_h) & \
                        (indices[:, 1] >= 0) & (indices[:, 1] < img_w)
