...
from utils.utils import (
        ...
        read_lines, load_mask)
from utils.image_funtions import smoothen
...
def get_locs(prefix, target_shape=None, domain="Unknown"):
    locs = load_tsv(f'{prefix}pseudo_locs.csv', domain=domain)
...
def get_gene_counts(prefix, reorder_genes=True, domain="Unknown"):
    cnts = load_tsv(f'{prefix}pseudo_st.csv', domain=domain)
    if reorder_genes:
...
def get_embeddings(prefix, domain="Unknown"):
    embs = load_pickle(f'{prefix}embeddings-hist.pickle', domain=domain)
    # embs = np.concatenate([embs['cls'], embs['sub'], embs['rgb']])
    # embs = embs.transpose(1, 2, 0)
    return embs

def get_label(prefix, domain="Unknown"):
    labels = load_pickle(f'{prefix}genes_3D.pkl', domain=domain)
    # embs = np.concatenate([embs['cls'], embs['sub'], embs['rgb']])
    # embs = embs.transpose(1, 2, 0)
    return labels

def get_mask(prefix, domain="Unknown"):
    mask  = load_mask(f'{prefix}mask.png', domain=domain)
    return mask
...
def main():
    ...
    impute(
            ...
            method='neural', prefix=prefix+'cnts-super-all/')
